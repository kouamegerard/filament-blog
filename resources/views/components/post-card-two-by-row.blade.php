    ...
    <div class="bg-white dark:bg-gray-800/40 backdrop-blur-2xl  rounded-2xl shadow-lg w-full relative p-4">
        <div class="grid grid-cols-12 sm:grid-cols-12 md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 gap-4">
            <div class="col-span-12 sm:col-span-6  md:col-span-6 lg:col-span-4 xl:col-span-4 ">
                {{-- <img src="assets/images/widgets/sm-1.jpeg" alt="" class="max-w-full h-auto rounded-xl"> [218.525px]--}}
                @if ($post->post_media())
                    <img src="{{ asset('storage/'.$post->id."/".$post->post_media()->file_name) }}" alt="{{ $post->title }}" class="max-w-full w-full h-[218.525px] object-cover rounded-xl">
                @else
                    <img src="assets/images/widgets/sm-1.jpeg" alt="{{ $post->title }}" class="max-w-full h-[218.525px] object-cover rounded-xl">
                @endif
            </div><!--end col-->
            <div class="col-span-12 sm:col-span-6  md:col-span-6 lg:col-span-8 xl:col-span-8 ">
                <div class=" h-full flex flex-col p-3">
                    <div class="w-full block">
                        <span class="text-[12px] bg-pink-500/10 text-pink-500 dark:text-pink-600 rounded font-medium py-1 px-2 inline-block mb-3" style="background-color: {{ $post->categories[0]->color }}">{{ $post->categories[0]->title }}</span>
                    ...
                    <div class="flex flex-wrap justify-between mt-auto">
                        <div class="flex items-center">
                            ...
                            <div class="ml-2">
                                <a tabindex="0" class="cursor-pointer hover:text-gray-500 focus:text-gray-500 text-gray-800 dark:text-gray-100 focus:outline-none focus:underline"><h5 class=" font-medium text-sm">Geneses New</h5></a>
                                <p tabindex="0" class="focus:outline-none text-gray-500 dark:text-gray-400 text-xs font-medium">Yoff, Dakar</p>
                            </div>
                        </div>
